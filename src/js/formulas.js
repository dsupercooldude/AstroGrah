// src/js/formulas.js
window.CITY_PRESETS = [ { name: "Dubai, UAE", lat: 25.2048, lon: 55.2708, utc: 4.0 }, { name: "Mumbai, India", lat: 19.0760, lon: 72.8777, utc: 5.5 }, { name: "New Delhi, India", lat: 28.6139, lon: 77.2090, utc: 5.5 }, { name: "Phalodi, India", lat: 27.1300, lon: 72.3600, utc: 5.5 }, { name: "Ujjain, India", lat: 23.1765, lon: 75.7885, utc: 5.5 }, { name: "London, UK", lat: 51.5074, lon: -0.1278, utc: 0.0 }, { name: "New York, USA", lat: 40.7128, lon: -74.0060, utc: -5.0 } ];
...
window.getKPLords = (lon) => { const nakIdx = Math.floor(lon / (360 / 27)); const remInNak = lon % (360 / 27); const subLordIdx = (Math.floor(remInNak / ((360 / 27) / 9)) + (nakIdx % 9)) % 9; return { starLord: window.PLANET_LORDS[nakIdx % 9], subLord: window.PLANET_LORDS[subLordIdx], subSubLord: window.PLANET_LORDS[(subLordIdx + 2) % 9] }; };
window.calcDasha = (moonDeg, dobStr) => { const nakLen = 360 / 27; const nakIdx = Math.floor(moonDeg / nakLen); const passedPct = (moonDeg % nakLen) / nakLen; let lordIdx = nakIdx % 9; const mahaYrs = window.VIMSHOTTARI_YEARS[window.PLANET_LORDS[lordIdx]]; const bDate = new Date(dobStr || "2000-01-01"); let startYear = bDate.getFullYear() + (bDate.getMonth() / 12) - (mahaYrs * passedPct); const periods = []; for (let i = 0; i < 9; i++) { const lrd = window.PLANET_LORDS[(lordIdx + i) % 9]; const dur = window.VIMSHOTTARI_YEARS[lrd]; periods.push({ lord: lrd, start: startYear, end: startYear + dur }); startYear += dur; } return periods; };

window.computeKundli = (profile, dateObj = null) => {
  if (!profile) return null;
  const targetDate = dateObj || new Date();
  const JD = window.julianDay(profile.dob, profile.time, profile.utcOffset); const T = (JD - 2451545) / 36525; const [Y, Mo] = (profile.dob || "2026-01-01").split("-").map(Number); const aya = window.getAya(Y + (Mo - 1) / 12);
  ...
  // FIX: KP Table mapped strictly to PLANETS, not Cusps!
  const kpPlanets = {};
  ...
  return {
    ...
    nak: window.NAKSHATRAS[Math.floor(sid.Moon / (360 / 27))], pada: Math.floor((sid.Moon % (360 / 27)) / ((360 / 27) / 4)) + 1,
    planetaryDegrees: sid, transits, dasha: window.calcDasha(sid.Moon, profile.dob), shadbala
  };
};
...
window.panchang = (dObj, ms = "amanta", utc = 5.5) => {
  ...
  return { tithi: ["Pratipada","Dwitiya","Tritiya","Chaturthi","Panchami","Shashthi","Saptami","Ashtami","Navami","Dashami","Ekadashi","Dwadashi","Trayodashi","Chaturdashi",isS ? "Purnima" : "Amavasya"][tIdx % 15], paksha: isS ? "Shukla" : "Krishna", masa, nak: window.NAKSHATRAS[Math.floor(ml / (360 / 27))], yoga: window.YOGAS[Math.floor(window.norm360(ml + sl) / (360 / 27))], karana, sr, ss, mr, msr, abh, chogDay, chogNight, horas, nightHoras, bhadra: bhadraApprox, rahu: getS(sr.getTime() + dMs * 0.8, dMs * 0.1), yamaganda: getS(sr.getTime() + dMs * 0.4, dMs * 0.1), gulika: getS(sr.getTime() + dMs * 0.2, dMs * 0.1), brahma: getS(sr.getTime() - dMs * 0.15, dMs * 0.08), vikram: dObj.getFullYear() + 57, saka: dObj.getFullYear() - 78 };
};
