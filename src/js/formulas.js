...
window.SIGN_TRAITS = {
  "Aries": "pioneering, dynamic, action-oriented", "Taurus": "stable, value-driven, steadfast", "Gemini": "intellectual, versatile, communicative",
  "Cancer": "intuitive, protective, deeply emotional", "Leo": "authoritative, charismatic, creative", "Virgo": "methodical, precision-driven, analytical",
  "Libra": "diplomatic, harmony-seeking, strategic", "Scorpio": "transformative, perceptive, resilient", "Sagittarius": "visionary, expansive, philosophical",
  "Capricorn": "disciplined, ambitious, structured", "Aquarius": "innovative, unconventional, forward-thinking", "Pisces": "empathetic, contemplative, intuitive"
};
...
// FIX: Strictly mapping KP array indexes to actual Planets
...
window.getAntardashas = (mahaLord, mahaStart, mahaEnd) => {
  const periods = []; const mahaYears = window.VIMSHOTTARI_YEARS[mahaLord] || (mahaEnd - mahaStart);
  let currentStart = mahaStart; let lordIdx = window.PLANET_LORDS.indexOf(mahaLord);
  for (let i = 0; i < 9; i++) {
    const antarLord = window.PLANET_LORDS[(lordIdx + i) % 9]; const antarYears = (mahaYears * window.VIMSHOTTARI_YEARS[antarLord]) / 120;
...
window.getPratyantarDashas = (antarLord, antarStart, antarEnd) => {
  ...
  for (let i = 0; i < 9; i++) {
    const pLord = window.PLANET_LORDS[(lordIdx + i) % 9]; const pYears = (antarYears * window.VIMSHOTTARI_YEARS[pLord]) / 120;
...
window.panchang = (dObj, ms = "amanta", utc = 5.5) => {
  ...
  const dMs = ss - sr;
  // FIX: Accurate Night Calculation
