  ...
  return (
    <div className="space-y-6 pb-12 gl-fadein mt-4">
      <div className="rounded-3xl border border-white/10 bgcard p-5 shadow-xl flex flex-col">
        <h3 className="font-serif text-base text-amber-200 mb-4 flex items-center gap-2"><i className="ph ph-planet" style={{ fontSize: 20 }}></i> Detailed Planetary Ledger & Longitudes</h3>
        <div className="overflow-x-auto mb-4">
          <table className="w-full text-xs font-mono text-left border-collapse">
            <thead>
              <tr className="border-b border-white/10 t50 text-[10px] uppercase">
                <th className="pb-3">Graha</th><th className="pb-3">Rashi (Sign)</th><th className="pb-3">Longitude</th><th className="pb-3">Nakshatra (Pada)</th><th className="pb-3">Motion</th>
              </tr>
            </thead>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {Object.entries(jaimini).map(([karaka, planet]) => {
              ...
              return (
                <div key={karaka} className="flex justify-between items-center bg-black/30 px-3 py-2 rounded-xl border border-white/5">
                  <span className="t60">{karaka}</span><span className="font-bold" style={{ color: pInfo.color }}>{planet}</span>
                </div>
              );
            })}
          </div>
        ...
        <div className="rounded-3xl border border-white/10 bgcard p-5 shadow-xl flex flex-col h-full">
            ...
            {Object.entries(avasthas).map(([planet, avastha]) => {
              ...
              return (
                <div key={planet} className="flex justify-between items-center bg-black/30 px-3 py-2 rounded-xl border border-white/5">
                  <span style={{ color: pInfo.color }} className="font-bold">{planet}</span><span className="t85 text-[11px]">{avastha}</span>
                </div>
              );
            })}
          </div>
          <div className="bg-black/40 border border-white/5 rounded-xl p-4 text-xs t85 leading-relaxed shadow-inner">
            ...
            ● <strong>Yuva (Youth):</strong> The planet is operating at 100% peak vibrancy, delivering active, powerful results.<br/><br/>
            ● <strong>Mrita (Dead) or Vriddha (Old):</strong> The planet's energy is dormant or fatigued. It requires patience and specific spiritual remedies to unlock.
          </div>
        </div>
      </div>
    </div>
