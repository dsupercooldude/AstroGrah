...
window.PersonTab = ({ pr, ch, date, setDate, onEdit, onPdf }) => {
  ...
  // FIX: Compute BioScores locally to resolve the 0% error
  const bioScores = window.bio ? window.bio(pr?.dob || "2000-01-01", date, pr?.utcOffset || 5.5) : { p: 0, e: 0, i: 0 };

  const weekday = window.WEEKDAY[date.getDay()];
  const gochara = window.generateDeepGochara ? window.generateDeepGochara(ch, ch.d1?.lagna || "Aries", date, weekday, bioScores) : {};
    ...
    <div className="space-y-6 pb-12 gl-fadein mt-4">
      ...
      <div className="bgcard rounded-3xl border border-white/10 p-6 shadow-xl flex justify-between items-start">
        ...
        <div className="flex gap-3">
          <button onClick={() => onPdf ? onPdf() : window.dispatchEvent(new CustomEvent('generate-pdf'))} className="w-10 h-10 rounded-full bg-emerald-500/10 text-emerald-400 flex items-center justify-center border border-emerald-500/30 hover:bg-emerald-500/20 transition shadow-lg">
            <i className="ph ph-file-pdf text-lg"></i>
          </button>
          <button onClick={() => onEdit ? onEdit() : window.dispatchEvent(new CustomEvent('open-edit-modal'))} className="w-10 h-10 rounded-full bg-white/5 text-white/70 flex items-center justify-center border border-white/10 hover:bg-white/10 transition shadow-lg">
      ...
      <div className="bgcard rounded-3xl border border-white/10 p-5 shadow-xl flex flex-col xl:flex-row justify-between items-center gap-4 relative z-20">
        ...
        <div className="flex flex-wrap justify-center items-center gap-2 bg-black/40 p-2 rounded-2xl border border-white/5 w-full xl:w-auto">
          {[{ l: "-1M", d: -30 }, { l: "-1W", d: -7 }, { l: "-1D", d: -1 }, { l: "Today", d: 0 }, { l: "+1D", d: 1 }, { l: "+1W", d: 7 }, { l: "+1M", d: 30 }].map(btn => (
            <button key={btn.l} onClick={() => { const nd = new Date(date); nd.setDate(nd.getDate() + btn.d); btn.d === 0 ? setDate(new Date()) : setDate(nd); }} className={`px-3 py-1.5 rounded-xl text-[10px] font-bold font-mono transition ${btn.d === 0 ? 'bg-amber-400/20 text-amber-400 border-amber-400/30' : 'bg-white/5 text-white/60 hover:bg-white/10'}`}>{btn.l}</button>
          ))}
        </div>
      </div>

      {/* KUNDALI CHART RENDERER (FIXED PROP LINKING) */}
      <div className="bgcard rounded-3xl border border-white/10 p-5 shadow-xl relative overflow-visible">
        ...
        <div className="flex justify-center items-center min-h-[300px] relative z-10">
          {/* FIX: Passed ac and kpTable so the chart knows what data to draw! */}
          {window.KundaliRenderer ? <window.KundaliRenderer ac={ch.d1} ch={ch} kpTable={ch.kpTable} chartStyle={formattedStyle} style={formattedStyle} /> : 
      ...
      {/* BIOCYCLE WIDGET WITH LOCAL SCORES */}
