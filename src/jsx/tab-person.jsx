    ...
    <div className="space-y-6 pb-12 gl-fadein mt-4">
      ...
      <div className="bgcard rounded-3xl border border-white/10 p-5 shadow-xl flex flex-col xl:flex-row justify-between items-center gap-4 relative z-20">
        <div className="flex items-center gap-4 w-full xl:w-auto">
          ...
          <div><div className="text-[10px] text-amber-400 font-mono tracking-widest uppercase mb-1">Active Prediction Horizon</div><div className="font-serif text-xl text-white font-bold">{weekday}, {date.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</div></div>
        </div>
        <div className="flex flex-wrap justify-center items-center gap-2 bg-black/40 p-2 rounded-2xl border border-white/5 w-full xl:w-auto">
          {[{ l: "-1M", d: -30 }, { l: "-1W", d: -7 }, { l: "-1D", d: -1 }, { l: "Today", d: 0 }, { l: "+1D", d: 1 }, { l: "+1W", d: 7 }, { l: "+1M", d: 30 }].map(btn => ( <button key={btn.l} onClick={() => { const nd = new Date(date); nd.setDate(nd.getDate() + btn.d); btn.d === 0 ? setDate(new Date()) : setDate(nd); }} className={`px-3 py-1.5 rounded-xl text-[10px] font-bold font-mono transition ${btn.d === 0 ? 'bg-amber-400/20 text-amber-400 border-amber-400/30' : 'bg-white/5 text-white/60 hover:bg-white/10'}`}>{btn.l}</button> ))}
        </div>
      </div>
            ...
            {Object.entries(ch.shadbala || {}).slice(0, 9).map(([planet, score]) => {
              ...
              return (
                <div key={planet} className="relative">
                  <div className="flex justify-between text-[10px] font-mono mb-1"><span className="font-bold flex items-center gap-1" style={{ color: pInfo?.color }}><span className="text-xs">{pInfo?.symbol}</span> {planet}</span><span className="t85 font-bold">{(score / 60).toFixed(1)} Rupas</span></div>
