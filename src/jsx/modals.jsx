...
window.SetupModal = ({ onConfig }) => {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center p-4 gl-fadein"><div className="w-full max-w-sm rounded-3xl bgcard2 p-6 shadow-2xl border border-white/10">
            <form onSubmit={async(e)=>{ e.preventDefault(); setErr(""); window.AppDB.setConfig(o,r,t); try{ await window.AppDB.callApi('GET',''); onConfig(); }catch(er){ if(er.message==="404") { onConfig(); } else { setErr("Token Invalid or Repo Missing."); window.AppDB.clearConfig(); } } }}>
                <window.SageLogo size={44}/><h2 className="text-center font-serif text-xl mt-2 mb-4 text-amber-200">Connect Cloud Vault</h2>
                ...
                <button type="button" onClick={()=>{ window.AppDB.enableLocal(); onConfig(); }} className="w-full text-xs t60 mt-3 hover:text-white transition">Skip Cloud - Use Offline Local Storage</button>
            </form></div></div>
    );
};
...
window.AuthModal = ({ onLogin }) => {
    ...
    const proceedToVault = async (normE, emailHash, reqChange, isMfaEnabled) => {
        const vaultFile = await window.AppDB.getFile(`gl_vault_${emailHash}.json`);
        const prof = typeof vaultFile.content.profiles === 'string' ? window.CryptoUtils.decrypt(vaultFile.content.profiles) : (vaultFile.content.profiles || []);
        const sett = typeof vaultFile.content.settings === 'string' ? window.CryptoUtils.decrypt(vaultFile.content.settings) : (vaultFile.content.settings || {});
    ...
    const handleSubmit = async (ev) => { 
        ...
        try { 
            const emailHash = await window.AppDB.hashKey(normE); let authFile = await window.AppDB.getFile('gl_auth.json'); if(!authFile.content.users) authFile.content.users = {};
            if(mode === "signup") { 
                if(authFile.content.users[emailHash]) throw new Error("Email already registered."); 
                const gen="Om-"+Math.random().toString(36).slice(-6)+"!"; const hashedPw = await window.CryptoUtils.hashPassword(gen); 
                authFile.content.users[emailHash] = { p: hashedPw, req: true }; await window.AppDB.saveFile('gl_auth.json', authFile.content, authFile.sha); 
                ...
                const hashedInput = await window.CryptoUtils.hashPassword(p); if(u.p !== p && u.p !== hashedInput) throw new Error("Invalid password."); 
    ...
    const handleMfaSubmit = async (ev) => {
        ev.preventDefault(); setErr(""); const normE = e.trim().toLowerCase();
        try {
            const emailHash = await window.AppDB.hashKey(normE); let authFile = await window.AppDB.getFile('gl_auth.json'); const u = authFile.content.users[emailHash];
            const secret = window.CryptoUtils.decrypt(u.mfa); if (!window.OTPAuth) throw new Error("Authenticator library missing.");
    ...
    if (mode === "mfa") return (
        <div className="min-h-screen flex items-center justify-center p-4 gl-fadein"><div className="w-full max-w-sm rounded-3xl bgcard2 p-6 shadow-2xl border border-white/10 text-center"><window.Icon name="shield-check" size={48} className="mx-auto text-emerald-400 mb-3" /><h2 className="font-serif text-2xl mt-1 mb-2 text-emerald-200">2FA Protected</h2><p className="text-xs t60 mb-5">Enter your 6-digit Authenticator app PIN.</p>
...
window.AdminAuthModal = ({ u, onClose, onAuthenticated }) => {
    ...
    const handleAuth = async (e) => {
        ...
        try { 
            let adminFile = await window.AppDB.getFile('gl_admin.json');
            if (!adminFile.content.adminUser) { const hashedDefault = await window.CryptoUtils.hashPassword(pwd); adminFile.content = { adminUser: adminUser.trim().toLowerCase(), p: hashedDefault, mfa: null }; await window.AppDB.saveFile('gl_admin.json', adminFile.content, adminFile.sha); alert("Admin Vault initialized with these credentials."); onAuthenticated(); return; }
...
window.AdminConsoleModal = ({ onClose, onResetDb }) => {
    const [o, setO] = useState(window.AppDB.config?.owner || ""); const [r, setR] = useState(window.AppDB.config?.repo || ""); const [t, setT] = useState(window.AppDB.config?.token || ""); const [adminMfaSetup, setAdminMfaSetup] = useState(null);
    const handleSaveDb = (e) => { e.preventDefault(); if(!confirm("Update Database Configuration?")) return; window.AppDB.setConfig(o, r, t); alert("Database configuration updated successfully."); onClose(); };
    ...
    const verifyAdmin2FA = async () => { const totp = new window.OTPAuth.TOTP({ secret: adminMfaSetup.secret }); if (totp.validate({ token: adminMfaSetup.pin, window: 1 }) === null) return alert("Invalid PIN."); let adminFile = await window.AppDB.getFile('gl_admin.json'); adminFile.content.mfa = window.CryptoUtils.encrypt(adminMfaSetup.secret); await window.AppDB.saveFile('gl_admin.json', adminFile.content, adminFile.sha); alert("Admin 2FA Activated Successfully."); setAdminMfaSetup(null); };
    ...
    return (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 p-4" onClick={onClose}><div onClick={e=>e.stopPropagation()} className="w-full max-w-md bgcard2 p-6 rounded-3xl border border-white/10 shadow-2xl gl-fadein max-h-[85vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4 border-b border-white/10 pb-3"><h3 className="font-serif text-lg text-amber-300 flex items-center gap-2"><window.Icon name="database"/> Dedicated Admin Console</h3><button onClick={onClose} className="hover:text-white t60"><window.Icon name="x"/></button></div>
