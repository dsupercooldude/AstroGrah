...
const bootInterval = setInterval(() => {
  ...
  const deps = {
    React: !!window.React, ErrorBoundary: !!window.ErrorBoundary, KundaliRenderer: !!window.KundaliRenderer,
    SetupModal: !!window.SetupModal, AuthModal: !!window.AuthModal, SettingsModal: !!window.SettingsModal,
    GhostPDFReport: !!window.GhostPDFReport, PersonTab: !!window.PersonTab, PanchangTab: !!window.PanchangTab,
    CompatTab: !!window.CompatTab, AskTab: !!window.AskTab, ReportsTab: !!window.ReportsTab, TabOrchestrator: !!window.TabOrchestrator
  };
  ...
  if (allLoaded) {
    ...
    function AppContent() {
      ...
      window.useIdleTimeout(() => {
        if (u) { try { localStorage.removeItem("gl_active_user"); } catch (e) {} setU(null); alert("Session timed out."); }
      }, 300000);
          ...
          if (dbC) {
            try {
              ...
              if (sess) {
                ...
                let profRaw = vaultFile.content.profiles; let prof = [];
                if (typeof profRaw === "string") { try { const decrypted = CryptoUtils.decrypt(profRaw); prof = typeof decrypted === "string" ? JSON.parse(decrypted) : decrypted; } catch (err) { prof = []; } } else { prof = profRaw || []; }
                if (!Array.isArray(prof)) prof = [];
                let settRaw = vaultFile.content.settings; let sett = {};
                if (typeof settRaw === "string") { try { const decryptedSet = CryptoUtils.decrypt(settRaw); sett = typeof decryptedSet === "string" ? JSON.parse(decryptedSet) : decryptedSet; } catch (err) { sett = {}; } } else { sett = settRaw || {}; }
      ...
      const chs = useMemo(() => {
        const o = {};
        if (Array.isArray(prs)) { prs.forEach((p) => { if (p && p.id && window.computeKundli) { o[p.id] = window.computeKundli(p, dt); } }); }
        return o;
      }, [prs, dt]);
      ...
      useEffect(() => {
        const handlePdf = async () => {
          ...
          if (!el) return;
          el.classList.remove('hidden'); el.style.left = '0';
          try {
      ...
      const fetchCityCoordinates = async () => { const query = formData.place; if (!query) return alert("Please type a city name first."); const preset = window.CITY_PRESETS?.find((c) => c.name.toLowerCase().includes(query.toLowerCase())); if (preset) { setFormData((prev) => ({ ...prev, place: preset.name, lat: preset.lat.toString(), lon: preset.lon.toString(), utcOffset: preset.utc.toString() })); return; } try { const res = await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(query)}&format=json&limit=1`); const data = await res.json(); if (data && data.length > 0) { const lat = parseFloat(data[0].lat); const lon = parseFloat(data[0].lon); const tz = calculateTimezone(lat, lon); setFormData((prev) => ({ ...prev, place: data[0].display_name.split(",")[0], lat: lat.toFixed(4), lon: lon.toFixed(4), utcOffset: tz })); } else { alert("City coordinates not found. Enter latitude and longitude manually."); } } catch (e) { alert("Network search failed. Enter coordinates manually."); } };
      ...
      const deleteProfile = async (id) => { if (!confirm("Are you sure you want to delete this profile?")) return; const nP = prs.filter((p) => p.id !== id); const vaultFile = await AppDB.getFile(`gl_vault_${u.emailHash}.json`); vaultFile.content.profiles = CryptoUtils.encrypt(nP); await AppDB.saveFile(`gl_vault_${u.emailHash}.json`, vaultFile.content, vaultFile.sha); setU({ ...u, profiles: nP }); if (nP.length > 0) setActiveProfileId(nP[0].id); setEd(null); };
      const updateSettings = async (ns) => { const vaultFile = await AppDB.getFile(`gl_vault_${u.emailHash}.json`); vaultFile.content.settings = CryptoUtils.encrypt(ns); vaultFile.content.profiles = vaultFile.content.profiles || CryptoUtils.encrypt(prs); await AppDB.saveFile(`gl_vault_${u.emailHash}.json`, vaultFile.content, vaultFile.sha); setU({ ...u, settings: ns }); };
      ...
      return (
        <div className="min-h-screen w-full font-sans pb-10 relative">
          ...
          <div className="bgcard2 border-b border-white/10 sticky top-0 z-30 shadow-lg">
            <div className="mx-auto max-w-md sm:max-w-3xl px-4 py-3 flex justify-between items-center pr-36">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => setAdminAuthOpen(true)} className="p-2 rounded-full border border-amber-400/30 bg-amber-400/10 hover:bg-amber-400/20 transition text-amber-300"><Icon name="database" size={17} /></button>
          ...
          {/* INVISIBLE RENDER TARGET FOR PDF EXPORT */}
